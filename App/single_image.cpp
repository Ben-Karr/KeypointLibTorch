...
int main(){
    ...
    std::string img_path = "../../data/sample_man.jpeg";
    cv::Mat img = cv::imread(img_path);
    if(img.empty()){
        std::cerr << "Frame is empty" << '\n';
    }
    double frame_width = img.cols;
    double frame_height = img.rows;
    ...
    img_src = draw_keypoints(img_src, keypoints, scores, true);

    cv::resize(img_src, img_src, cv::Size(int(img_size * ratio), img_size), cv::INTER_LINEAR); // restore input image ratio
    cv::imshow("Keypoint detection", img_src);
    cv::waitKey(0);

    cv::imwrite("../sample_with_keypoints.jpg", img_src);
    return 0;
}
